## Simple log simulator that sends logs data for single vin number
import argparse
import random
import time
import json
...
def generate_random_data_entry():
    data_options = [
        ("CPU_Load", "Unsigned Short Int", random.randint(0, 100)),
...
def produce_messages(topic, bootstrap_servers):
    ...
    Vin_list = ["111111", "222222", "333333"]

    vin_number = random.choice(Vin_list)  # Random select VIN number

    # Track total count of DataEntries
    total_data_entries_count = 0

    while True:
        print(f"Producing data for VIN: {vin_number}")

        # Generate campaign data
        campaign_data = generate_campaign_data(vin_number)
